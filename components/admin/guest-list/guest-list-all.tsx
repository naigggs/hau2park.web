...
export function AllGuestList() {
  ...
  const getStatusClasses = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'rejected':
      case 'declined':
        return "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/10 dark:text-rose-400 dark:border-rose-800/30";
      case 'open':
        return "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-950/10 dark:text-sky-400 dark:border-sky-800/30";
      default:
              ...
              <Card className="overflow-hidden border-border/40 hover:border-border hover:shadow-sm transition-all max-w-full">
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge 
                      variant="outline" 
                      className={`font-normal ${statusClasses}`}
                    >
                      {guest.status}
                    </Badge>
                  </div>
